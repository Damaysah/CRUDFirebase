package com.damsah.daftarkaryawan
...
import com.damsah.daftarkaryawan.databinding.ActivityMyListDataBinding
...
class MyListData : AppCompatActivity(), RecyclerViewAdapter.dataListener {
    ...
    private fun GetData() {
        ...
        getReference.child("Admin").child(getUserID).child("DaftarKaryawan")
    ...
    override fun onDeleteData(data: data_teman?, position: Int) {
        ...
        if (getRefference != null) {
            getRefference.child("Admin").child(getUserID).child("DaftarKaryawan").child(data?.key.toString()).removeValue()
